...
const NewsDetail = () => {
  ...
  if (!article) return <div className="min-h-screen pt-24 text-center py-40">Article not found.</div>;

  return (
    <div className="min-h-screen pt-24">
      <article className="container mx-auto px-4 py-16 max-w-3xl">
        <Link to="/news" className="inline-flex items-center gap-2 text-primary font-medium mb-6 hover:gap-3 transition-all">
        ...
        <GlassCard className="overflow-hidden" hover={false}>
          ...
          <div className="p-8">
            <div className="flex items-center gap-3 mb-4">
              <span className="text-xs font-medium gradient-bg text-primary-foreground px-3 py-1 rounded-full">{article.category}</span>
              <span className="text-sm text-muted-foreground flex items-center gap-1"><Calendar className="w-3 h-3" />{article.date}</span>
            </div>
            <h1 className="text-3xl font-heading font-bold mb-4">{article.title}</h1>
...
const News = () => (
  <div className="min-h-screen pt-24">
    <section className="container mx-auto px-4 py-16">
      <SectionTitle title="School News" subtitle="Stay updated with the latest happenings at ABC International" />
            ...
            <GlassCard className="overflow-hidden group h-full">
              ...
              <div className="p-6">
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-xs font-medium gradient-bg text-primary-foreground px-2.5 py-1 rounded-full">{news.category}</span>
                  <span className="text-xs text-muted-foreground flex items-center gap-1"><Calendar className="w-3 h-3" />{news.date}</span>
                </div>
                <h3 className="font-heading font-semibold mb-2 group-hover:text-primary transition-colors">{news.title}</h3>
